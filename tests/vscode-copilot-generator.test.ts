...
describe('VSCodeCopilotGenerator', () => {
    ...
    beforeEach(() => {
        vi.resetAllMocks();
        vi.mocked(templateUtils.getTemplateRoot).mockResolvedValue('/mock/templates');
    });
    ...
    it('should generate VS Code Copilot specific files', async () => {
        ...
        vi.mocked(templateUtils.loadTemplate).mockImplementation(async (path) => {
            if (path === 'commands/setup.toml') {
                return 'description = "Setup Conductor"\nprompt = "Run setup"';
            }
            return 'description = "Other"\nprompt = "Other"';
        });

        vi.mocked(templateUtils.substituteVariables).mockImplementation((content) => content);

        await generator.generate(targetDir);

        // Verify directories created
        expect(fs.ensureDir).toHaveBeenCalledWith(join(targetDir, '.github/prompts'));
        expect(fs.ensureDir).toHaveBeenCalledWith(join(targetDir, '.github/conductor'));
        ...
        const expectedFile = join(targetDir, '.github/prompts/conductor:setup.prompt.md');
