...
describe("KiloCodeGenerator", () => {
	let generator: KiloCodeGenerator;
	...
	describe("validate", () => {
		...
		it("should throw if conductor is already installed", async () => {
			...
			await expect(generator.validate(targetDir)).rejects.toThrow(
				"Conductor (Kilo Code) is already installed",
			);
		});
	});
	...
	describe("generate", () => {
		...
		it("should generate command files with markdown title format", async () => {
			const mockToml =
				'description = "Test command"\nprompt = "Test prompt content"';
			(loadTemplate as ReturnType<typeof vi.fn>).mockResolvedValue(mockToml);
