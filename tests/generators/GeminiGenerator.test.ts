...
describe("GeminiGenerator", () => {
	...
	describe("validate", () => {
		it("should return targetDir if valid with project scope", async () => {
			...
			await expect(generator.validate(targetDir, "project")).resolves.toBe(
				targetDir,
			);
		});
		...
		it("should throw if target directory does not exist", async () => {
			(fs.existsSync as ReturnType<typeof vi.fn>).mockReturnValue(false);
			await expect(
				generator.validate("/nonexistent", "project"),
	...
	describe("generate", () => {
		it("should create directories based on config", async () => {
			...
			expect(filePath).toBe(
				join(targetDir, ".gemini", "commands", "conductor", "setup.toml"),
			);
