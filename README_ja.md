...
**仕組み:**
1. 初回実行時に、対話的またはフラグで設定を選択します
2. 2回目以降の実行では、保存された設定が自動的に適用されます
3. CLI フラグは常に保存された設定を上書きします
...
保存された設定をクリアし、すべてのプロンプトを最初から表示するには:
...
設定は XDG Base Directory Specification に従って保存されます:
